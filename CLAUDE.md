...
**The repo is documentation only right now — there is no `src/`.** Phase 1 is eleven milestones,
M0 to M11, defined in [docs/build-handoff.md](docs/build-handoff.md) section 4. Build one milestone per
session, in order. Each milestone's acceptance criteria are the gate: do not start one until the
...
- **Determinism.** All randomness inside the service flows through `Seeded` in `expansion/seeded.py`,
  ...
  `datetime.now()` in any generation or expansion path. Run ids are generated by the *client*, so
  `uuid4()` is correct there and only there.
...
- `uv run pytest -m integration --store postgres` — integration against one backend (`sqlite`, `postgres`, `mongo`)
- `uv run ruff check` and `uv run mypy src` — both must pass before any commit
- `docker compose --profile local up` — service plus Mongo, for authoring; `--profile shared` for Postgres
- `--store sqlite <path>` — containerless mode, and what CI uses
