...
**`external_refs` is `{}` in phase 1, and `run_get` returns it that way.** It is free-form on purpose — the model's docstring says "so M10 can add a target without a model change" — and this example showed `{"otel_trace_id": null, "langfuse_run_id": null}` until M10, which advertised a two-key shape no tool ever served. Nothing writes it: `run_export` derives its `external_ref` as `blake2b(run_id)`, so the value is already implied by the run and re-exporting yields the same one. A client must therefore read this as an **open map that is empty today**, not as two nullable keys — `external_refs.get("otel_trace_id")` is the only safe access, and it will be absent rather than null. `DECISIONS.md` [M10] records why no narrow store write was added at the last milestone of the phase, and what it would cost to add one later.
...
M8 added **one**: `run_already_finished`, attached by `fetch_step` when the run's lifecycle is closed. Ruling R-54(c) settles that a finished run still serves — "M8 may add a warning if it proves useful; it must not add a refusal" — and this is the warning half, ratified by R-67(a). Its detail carries `run_id`, `status` and `finished_at`, and deliberately no `node_id`, so R-54(b)'s merge key records it once per run rather than once per step. It is on the **read path only**: the code means "a finished run served you a fixture anyway", and a write is not serving, so neither `record_step` nor `run_finish` attaches it.

**Three more were added and lost**, all on the two M8 writes, and they are recorded here rather than quietly reverted so that none is reinvented. `step_actual_conflict` and `run_finish_mismatch` were warnings on a *successful* response for a re-write whose value **differed**, argued from R-53 and ground rule 3. Ruling R-65 replaced both with `AP-007` on an `ok: false` envelope (section 3.5): the write **did not happen**, `ok: true` misreports what the store holds, and `set_step_actual` already *raises* (R-33), so a tool reporting success would be claiming one the storage layer declined to give. `step_actual_already_recorded` was then added for the *identical* half, reading R-65's first draft literally — and went when **R-65 was amended**: a caller repeating a write is a client retrying after a timeout, so success is the *expected* outcome, and a vocabulary that fires on expected outcomes trains callers to ignore it. (The draft's own precedents pointed the same way: R-29's byte-identical re-publish is silent and R-47's compare-and-set loser errors.)

So a repeated write has exactly two outcomes and no warning distinguishes them: "the value is already yours" is a **silent** success, and "someone else's value is there" is `ok: false` with `AP-007`.
...
**Not rules, and never registered as ones.** Three failures a user can cause happen before or after a document exists, so no `BP-*`/`DS-*` rule can own them; two more are resolution rather than validation, which section 1's envelope covers explicitly ("on a validation **or resolution** failure"); the sixth is a deterministic id space that has no free value left, the seventh is a write-once value that is already recorded and differs from the one supplied, and the eighth is an outward export target that refused the trace or was not listening. They appear in `rule`, carry a pointer at the offending *argument*, and are `error` severity. Added at M4, extended at M5 by ruling R-49(b), at M8 by R-65 and at M10 by `run_export`; `service/envelope.py` is the implementation and `tests/unit/test_service_envelope.py` asserts they are disjoint from the registry and from this catalogue.
...
| AP-008 | An **outward export target** refused the trace, or is not listening. Nothing was published and nothing about the run changed. Added at M10 for `run_export`. Distinct from `AP-001`, which is a malformed argument — an unknown `target` *is* `AP-001`, and this is the case where the target was valid and the collector at the other end was not there. Distinct from `AP-005`, which is *this service's* store refusing through a programming-error guard: a collector refusing a batch is a fact about someone else's deployment, and the only actionable thing in it is the endpoint, which the finding's `context` carries (ruling R-60: put the URL beside the count). Reporting it as `ok: true` would claim a publication that did not happen — ruling R-65's rule pointed outward — and it is **not gating**, because ground rule 3 governs refusing to *serve* and an export is a write |
...
| `record_step` | `run_id: str`, `node_id?: str`, `tool_name?: str`, `iteration?: int`, `actual: object` | `{ok}`, plus the stored step and the `resolved_node_id` under the `record` key. Addressed exactly as `fetch_step` is, and keyed on the **resolved** step (ruling R-64), so a step fetched by tool name can be recorded by node id. **Write-once per key** (ruling R-33), split by ruling R-65: an identical re-record is a **silent** no-op success, a **differing** one is `AP-007` and nothing is written, and an actual for a step that was never served is `AP-004`. **Never grades** — `actual` is stored verbatim and is checked against neither `expected.final` nor the node's `output_schema`, because ground rule 2 puts comparison in the client. Built at M8 by ruling R-15 |
| `run_finish` | `run_id: str`, `outcome: object`, `status: str` | The stored run. `status` is `finished` or `abandoned`; anything else, `running` included, is `AP-001` naming the vocabulary. A compare-and-set, so the **first** close wins (ruling R-65): an identical repeat is a **silent** no-op success — a retry is meant to be indistinguishable from the call it retries — and a divergent one is `AP-007` with nothing written, so the recorded outcome stays what it was. `finished_at` comes from the injected clock (ruling R-09). **Never grades** — `outcome` is stored even when the blueprint's `outcome_schema` would reject it. A closed run still serves `fetch_step` (ruling R-54(c)), with a `run_already_finished` warning. Built at M8 by ruling R-15 |
| `run_get` | `run_id: str` | Full run. `steps` and the reconstructed `path` are **deterministically ordered** by `(seq, node_id, iteration)` (ruling R-37). `external_refs` is **always `{}`** in phase 1 — nothing writes it, because `run_export`'s ref is derived from the run id (see section 2.3 and the `run_export` row) |
| `run_evidence` | `run_id: str` | The evidence bundle, under the `evidence` key: `{run, pin, dataset, comparison, expected, outcome_schema, actual, nodes[], path, warnings[]}`. Read-only, and a **strict** read — the guard in `tests/unit/test_runtime_is_read_only.py` asserts it reaches no store mutator at all, not even the five run writes. `outcome_schema` is the pinned blueprint's schema **itself and not a reference to it** (ruling R-82), because M10's gate requires the bundle to be sufficient for the client's three comparison helpers with no further server call and `compare.schema(instance, outcome_schema)` takes the schema as an argument. The bundle's contents are enumerated by those three signatures: `exact`/`subset` need `expected.final` and `actual`, `schema` needs `outcome_schema`, `grade` needs `comparison`. `nodes` is one entry per **served step** — per `(node_id, iteration)` — each carrying `expected` (the authored fixture's `output`) beside `actual` (what `record_step` recorded), so the same helpers work a level down. `path` carries `expected` and `actual` side by side and **computes no verdict**: ground rule 2 is "there is no comparison logic in the server", and `exact(path.expected, path.actual)` in the client is the comparison. Built at M10 by ruling R-15 |
...
| `run_export` | `run_id: str`, `target: str` | `{external_ref}`, where `external_ref` is `{target, trace_id, endpoint, spans}`. Publishes the run to an **OTLP collector** as one trace with one span for the run and one per served step. `target` is a closed vocabulary — `otel` for a vendor-neutral trace, `langfuse` for the same trace plus the `langfuse.*` experiment and item attributes that make it arrive as a dataset run (ruling R-83: attributes, **not** a vendor SDK) — and anything else is `AP-001` naming both. The endpoint comes from the OpenTelemetry SDK's own `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` / `OTEL_EXPORTER_OTLP_ENDPOINT` convention, so agent-props adds no configuration of its own and **never reads a credential** (ground rule 4): an API key travels in `OTEL_EXPORTER_OTLP_HEADERS`, which the exporter reads and no module in `src/` mentions. The trace id is `blake2b(run_id)`, so exporting one run twice names the same trace and the `external_ref` is reproducible — which is why nothing is stored and `Run.external_refs` stays empty in phase 1. A collector that refuses or is not listening is `AP-008` with the endpoint in its context. Built at M10 by ruling R-15 |

Phase 1 ships `run_start` and `fetch_step` as reads, and `record_step` and `run_finish` as the only two writes a running agent can make — both to the **run**, never to a dataset, which is the division PRD 5.6 draws and `tests/unit/test_runtime_is_read_only.py` enforces over the call graph. `run_evidence` and `run_export` land at M10 by ruling R-15, and both are reads: the storage shape did not change for them, which is what modelling `external_refs` early bought.
