...
M8 added three, all on the two run writes ruling R-15 lands there. `step_actual_conflict`, attached by `record_step` when a **different** actual is already recorded for the step: `set_step_actual` is write-once per key (ruling R-33) and refuses at the store, and this is how that refusal reaches a caller, because a step's recorded actual is evidence and the service never gates. Its detail carries `run_id`, `node_id`, `iteration`, `recorded_at` and `differs`, and the response carries the *stored* step, so a caller that ignored the warning still cannot mistake its own document for the recorded one. `run_finish_mismatch`, attached when `run_finish` names a run that is already closed with a different `status` or `outcome` — R-53's answer to a diverging `run_start`, one tool along: the first close wins, the divergence is reported, and a repeated *identical* finish adds no warning at all so a retry after a network blip is silent. Its detail carries `diverged`, `recorded` and `requested`. And `run_already_finished`, attached by `fetch_step` and `record_step` when the run's lifecycle is closed: ruling R-54(c) settles that a finished run still serves — "M8 may add a warning if it proves useful; it must not add a refusal" — and this is the warning half. Its detail carries `run_id`, `status` and `finished_at`, and deliberately no `node_id`, so R-54(b)'s merge key records it once per run rather than once per step.
...
| `record_step` | `run_id: str`, `node_id?: str`, `tool_name?: str`, `iteration?: int`, `actual: object` | `{ok}`, plus the stored step and the `resolved_node_id` under the `record` key. Addressed exactly as `fetch_step` is, and keyed on the **resolved** step (ruling R-64), so a step fetched by tool name can be recorded by node id. Write-once per key: an identical re-record is a no-op success, a differing one keeps the stored actual and warns with `step_actual_conflict`, and an actual for a step that was never served is `AP-004` (ruling R-33). **Never grades** — `actual` is stored verbatim and is checked against neither `expected.final` nor the node's `output_schema`, because ground rule 2 puts comparison in the client. Built at M8 by ruling R-15 |
| `run_finish` | `run_id: str`, `outcome: object`, `status: str` | The stored run. `status` is `finished` or `abandoned`; anything else, `running` included, is `AP-001` naming the vocabulary. A compare-and-set, so the **first** close wins: an identical repeat is a no-op success and a divergent one keeps the recorded outcome and warns with `run_finish_mismatch`. `finished_at` comes from the injected clock (ruling R-09). **Never grades** — `outcome` is stored even when the blueprint's `outcome_schema` would reject it. A closed run still serves `fetch_step` (ruling R-54(c)), with a `run_already_finished` warning. Built at M8 by ruling R-15 |
