...
[`docs/`](.) for the full specification: [`docs/prd.md`](prd.md) (product rationale),
[`docs/build-handoff.md`](build-handoff.md) (stack, repo layout, milestone plan),
[`docs/contracts.md`](contracts.md) (schemas, rule catalogue, tool signatures), and
[`docs/worked-example.md`](worked-example.md) (the fixtures under `tests/fixtures/`).

## Status

Phase 1 is complete: M0 through M10 plus M9.5, the milestones defined in
[build-handoff.md](build-handoff.md) section 4. Twenty-seven tools, four prompts, six
resources, three storage backends, two installable packages and one web app. M11, the
TypeScript client, was descoped by the owner and is not built.

[spec-rulings.md](spec-rulings.md) is authoritative where it disagrees with any other
document: it records the corrections applied after a consistency scan found 35 disagreements
between the four specifications.

## Use it against your own agent repo

**Handing this to a coding agent in another repository?**
[`docs/connect-your-agent-repo.md`](connect-your-agent-repo.md) is that brief — the MCP config
...
Every tool answers one of the two envelopes in [`docs/contracts.md`](contracts.md) section 1,
