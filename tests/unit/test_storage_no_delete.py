...
#: Every way SQLAlchemy Core, the ORM or Alembic can express row deletion.
#:
...
DELETING_CALLS = frozenset({"delete", "delete_all", "bulk_delete", "truncate"})
...
def test_no_adapter_declares_a_delete_method() -> None:
    """The Protocol is a minimum, so an adapter could add one. None does."""
    offenders = [name for name in dir(SqlStore) if name.startswith("delete")]
    assert offenders == [], f"SqlStore declares a delete method: {offenders}"
